document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentTreeContextMenuListener = null;
  let tooltipVisible = false;
  const commonNodeIds = ["c1-g"];

  let tabs;
  let tabContents;
  let tabOrder;
  let availablePointsEl;
  let availablePointsDisplay;
  let cursorFollower;
  let tooltip;
  let resetAllBtn;
  let pointsInput;
  ...
  fetch("skillData.json")
    ...
    .then((data) => {
      // Store data in our global variables
      SKILL_TREES = data.skillTrees;
      skillData = data.skillData;
      nodeIconMap = data.nodeIconMap;
    ...
    .catch((error) => {
      ...
      document.body.innerHTML =
        '<div style="color:red;padding:20px;">Error loading skill data. Please check console for details.</div>';
    });
  ...
  function initializeApp() {
    // Get DOM elements
    tabs = document.querySelectorAll(".skill-tabs .tab-item");
    tabContents = document.querySelectorAll(".skill-tree-content");
    tabOrder = Array.from(tabs).map((tab) => tab.dataset.tabId);
    availablePointsEl = document.getElementById("available-points");
    availablePointsDisplay = document.querySelector("#available-points span");
    resetAllBtn = document.getElementById("reset-all-points");
    pointsInput = document.getElementById("available-points-input");
    applyBtn = document.getElementById("apply-points");

    // Initialize available points from HTML
    availablePoints = parseInt(
    ...
    // Apply icons to the skill nodes
    applySkillIcons();

    // Set up tooltips
    setupTooltips();

    // Add event listeners (must be after DOM elements are defined)
    setupEventListeners();
